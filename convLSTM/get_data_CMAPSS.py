...
def TrainImages(n_set,n_flights ,filter_t,f_modes):
  ...
  set = './datafolder/train/train_FD00'+str(n_set)+'.txt'
...
def RealImages(n_set, filter_t, f_modes):
    # Obtain only the last image of each flight in the dataset
    # In this case, we do not extract the RUL, since is given in a separate file
    images = []
    ...
    #flight = Test_data[Test_data['Unit Number'] == flights_num]
    if f_modes:
        X = Test_data.drop(
            ['Unit Number',  # 'Operational Setting 1', 'Operational Setting 2', 'Operational Setting 3',
             'Sensor Measurement  1', 'Sensor Measurement  5', 'Sensor Measurement  6', 'Sensor Measurement  10',
    ...
    length = len(X)
    if filter_t > length:
        print("wrong, there are too little inputs!")
...
def get_data(window,f_modes=True,data1=False,if_test=True):
    if f_modes:
        ...
        if(if_test):
            ...
            X_test1, Y_test1 = RealImages(1, window, f_modes=f_modes)
            X_test2, Y_test2 = RealImages(2, window, f_modes=f_modes)
        ...
        if(if_test):
            ...
            X_test3, Y_test3 = RealImages(3, window, f_modes=f_modes)
        ...
        if(if_test):
            ...
            X_test4, Y_test4 = RealImages(4, window, f_modes=f_modes)
