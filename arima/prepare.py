...
def stationarity_test(dataset,number):
  ...
  message = '原始序列经过'+str(diff)+'阶差分后归于平稳，p值为'+str(adf[1])+'\n'
  return message
...
#白噪声检测
  ...
  if p < 0.05:
    ...
    message+='原始序列为非白噪声序列，对应的p值为：'+str(p)+'\n'
  else:
    print(u'原始该序列为白噪声序列，对应的p值为：%s' %p)
    message+='原始该序列为白噪声序列，对应的p值为：'+str(p)+'\n'
  [[lb], [p]] = acorr_ljungbox(data['rentNumber'].diff().dropna(), lags = 1)
  if p < 0.05:
    print(u'一阶差分序列为非白噪声序列，对应的p值为：%s' %p)
    message+='一阶差分序列为非白噪声序列，对应的p值为：'+str(p)+'\n'
  else:
    print(u'一阶差分该序列为白噪声序列，对应的p值为：%s' %p)
    message+='一阶差分该序列为白噪声序列，对应的p值为：'+str(p)+'\n'
  return message
...
def plot_results(predicted_data, true_data):
  ...
  plt.legend()
  plt.savefig('./result_img/arima_example.png')
  # plt.show()
  plt.close('all')
